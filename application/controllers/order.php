...

{
	...
	function add()
	{
		if( $this->uri->segment(3) ){
			...
			$data["action"] = "insert";
			$data["order"] = null;
			$data["methodPairs"] = $this->order_model->fetch_value_list("poOrderMethod", "poOrderMethod");
			$data["methodPairs"]["other"] = "Other...";
			$data["typePairs"] = $this->order_model->fetch_value_list("poPaymentType", "poPaymentType");
	...
	function vendor_orders()
	{
		...
		$data["kVendor"] = false;
		$data["title"] = "List of Orders"; //seems unwise to allow display of all orders

		if($this->uri->segment(3)){
			$kVendor = $this->uri->segment(3);
			$vendorName = $this->vendor_model->get_vendor_value($kVendor, "vendorName");
	...
	function find_records()
	{
		...
		$data["orders"] = $this->order_model->fetch_by_date_range($startDate, $endDate);
