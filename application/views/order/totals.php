...
foreach ($orders as $order) {
    ...
    if ($lastPO != $order->kPO) {
        if ($notFirst == true && $lastPO != $order->kPO) {
            $orderLines[] = "<tr><td colspan='7' class='totals'>Order Total: " .
                     getAsCash($poTotal) . "</td></tr>";
        }
        $line = sprintf(
                "<tr><td colspan='7' class='poLine'><a href='%s'>%s</a> Date: %s</td></tr>", 
                site_url("order/view/$order->kPO"), $order->kPO, 
    ...
    $orderLines[] = "<td class='totals'>" . getAsCash($order->itemPrice) .
             "</td>";
...
$orderLines[] = "<tr><td colspan='7' class='totals'>Grand Total: " .
         getAsCash($grandTotal) . "</td></tr>";
$orderLines[] = "</table>";
...
$categoryLines[] = "<tr><td colspan='2' class='totals'>Grand Total: " .
         getAsCash($grandTotal) . "</td></tr>";
$categoryLines[] = "</table>";
