...
if (count($orders) > 0) {
    foreach ($orders as $order) {
        ...
        $item[] = getAsCash($order->orderTotal);
        $item[] = "<a class='button' href='" . site_url(
                "order/view/$order->kPO") . "'>View</a>";
        $row["table"][] = "<tr><td>" . implode("</td><td>", $item) . "</td></tr>";
        $vendor_total += $order->orderTotal;
    }
    $listing[] = $row;
} else {
    ?>
<fieldset
	...
	<legend
		class='warning'
		style='padding: 5px'>There were no orders found.</legend>
...
<fieldset>
	...
	<p>
		Grand Total: <strong><?=getAsCash($vendor_total);?></strong>
	</p>
</fieldset>
<? endforeach;