...
class Order_model extends CI_Model{
	...
	function prepare_variables()
	{
		...
		if($this->input->post('poTotal')){
			$this->poTotal = $this->input->post('poTotal');
		}
	...
	{
		$this->db->select("vendor.kVendor as kVendor, vendor.vendorName as vendorName, order.poDate as poDate, order.poTotal as poTotal, order.kPO as kPO");
	...
	function fetch_value_list($fieldName, $valueName){
		...
		$fieldPairs = getKeyedPair($fieldList, array($fieldName, $valueName));
		return $fieldPairs;
	}
	...
	{
		//SELECT * FROM po , item WHERE po.kPO=item.kPO AND (po.poDate between '2010-07-01' AND '2011-06-30') order by po.kPO,po.poDate
